...
def collate_func(batch: Sequence[Tuple[Union[torch.Tensor, str]]],
                 ...
                 pad_token_id, length_max ):
    imgs, captions1 = zip(*batch)

    # それぞれのサンプルの5個のキャプションの中から1つを選択してトークナイズ
    #captions = [tokenize_caption(
    #    random.choice(cap), word_to_id) for cap in captions]
    captions = []
    for cap in captions1:
        caption = tokenizer.encode( random.choice( cap ) )
        caption.append( eos_token_id )
        caption.append( eos_token_id )
    ...
    targets = torch.full((len(captions), length_max),
                         pad_token_id, dtype=torch.int64)
...
def collate_func5(batch: Sequence[Tuple[Union[torch.Tensor, str]]],
                 tokenizer, eos_token_id, pad_token_id, length_max):
    imgs, captions1 = zip(*batch)
    
    imgs1 = []
    imgs2 = []
    captions = []
    for (cap, img ) in zip( captions1, imgs ) :
        imgs1.append( transforms(img) )
        imgs2.append( transforms2(img) )   
        caption = tokenizer.encode(random.choice(cap))
        caption.append( eos_token_id )
        caption.append( eos_token_id )
        captions.append( caption )
    ...
    #captions = [tokenize_caption(
    #    random.choice(cap), word_to_id) for cap in captions]
    #captions = [tokenizer.encode(random.choice(cap)) for cap in captions ] 
    #captions.append( eos_token_id )
    #captions.append( eos_token_id )
    ...
    #print( "DEBUG: 1 captions:", captions )
    lengths = [ len( cap ) for cap in captions]
    targets = torch.full((len(captions), length_max),
                         pad_token_id, dtype=torch.int64)
    for i, cap in enumerate(captions):
...
def collate_func6(batch: Sequence[Tuple[Union[torch.Tensor, str]]],
                 tokenizer, eos_token_id, pad_token_id, length_max):
    imgs, captions1 = zip(*batch)
